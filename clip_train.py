...
wandb.init(project="clip_liquify", config=CONFIG)
CONFIG = wandb.config
...
def load_split_train_test(datadir, valid_size=.125):
    ...
    trainloader = torch.utils.data.DataLoader(train_data, sampler=train_sampler, batch_size=1024,
                                              pin_memory=True, drop_last=False, num_workers=8)
    testloader = torch.utils.data.DataLoader(test_data, sampler=test_sampler, batch_size=1024,
                                             pin_memory=True, drop_last=False, num_workers=8)
...
data_path='/home/ai-team/members/jayant/liquify_classifier/Classifier'
...
if os.path.exists(features_cache):
    ...
    os.makedirs('ViT-B',exist_ok=True)
    trainloader, testloader = load_split_train_test(data_path, .2)
...
for epoch in range(1, CONFIG["epochs"]+1):
    ...
    with tqdm(total=len(feat_loader_train), desc=f"Epoch {epoch:>3}/{CONFIG['epochs']}") as pbar:
        ...
        if accuracy > global_accuracy:
            ...
            torch.save(cls_head.state_dict(), f"{weight_path}/weights_{epoch}.pth")
